...
public class VectorizedPropertyDecoder {
        ...
        case BOOLEAN:
          {
            ...
            return presentStream != null
                ...
                : new BooleanFlatVector(column.getName(), dataVector, dataStreamMetadata.numValues());
          }
        case UINT_32:
        case INT_32:
          {
    ...
    if (column.hasScalarType()) {
      ...
      var scalarType = column.getScalarType();
        return switch (scalarType.getPhysicalType()) {
            ...
            case UINT_32, INT_32 -> decodeIntColumn(data, offset, column, scalarType, nullabilityBuffer);
  ...
  private static BooleanFlatVector decodeBooleanColumn(byte[] data, IntWrapper offset, MltTilesetMetadata.Column column, int numFeatures, BitVector nullabilityBuffer) {
    var dataStreamMetadata = StreamMetadataDecoder.decode(data, offset);
    var vectorType =
        VectorizedDecodingUtils.getVectorTypeBooleanStream(
  ...
  private static Vector<LongBuffer, Long> decodeLongColumn(byte[] data, IntWrapper offset, MltTilesetMetadata.Column column, BitVector nullabilityBuffer, MltTilesetMetadata.ScalarColumn scalarType) {
    ...
    if (vectorType.equals(VectorType.FLAT)) {
      if(isNullable){
        ...
        var dataStream = VectorizedIntegerDecoder.decodeLongStream(data, offset, dataStreamMetadata, isSigned);
  ...
  private static Vector<IntBuffer, Integer> decodeIntColumn(byte[] data, IntWrapper offset, MltTilesetMetadata.Column column, MltTilesetMetadata.ScalarColumn scalarType, BitVector nullabilityBuffer) {
    ...
    if (vectorType.equals(VectorType.FLAT)) {
      if(isNullable){
        ...
        var dataStream = VectorizedIntegerDecoder.decodeIntStream(data, offset, dataStreamMetadata, isSigned);
